...
function generateContract(app) {
  ...
  if (!fs.existsSync(baseDir)) {
    app.logger.warn('[egg-swagger-docs] can not found contract in app`s directory');
  }
  ...
  for (let object in source) {
    ...
    for (let field in def) {
      ...
      if (!def[field].hasOwnProperty('type')) {
        throw new Error(`[egg-swagger-docs] ${path}: ${object}.${field}.type is necessary`);
      }
      ...
      if (def[field].type === 'array') {
        if (!def[field].hasOwnProperty('itemType')) {
          throw new Error(`[egg-swagger-docs] ${path}: ${object}.${field}.itemType is necessary`);
        }
