...
module.exports = {
  ...
  basicRouterRegister: app => {
    ...
    app.logger.info('[egg-swagger-docs] register router: get /swagger-doc');

    // swaggerUI的静态资源加入缓存，配置访问路由
    const swaggerH5 = path.join(__dirname, '../../app/public');
    app.use(staticCache(swaggerH5, {}, {}));
    app.logger.info('[egg-swagger-docs] register router: get /swagger-ui.html');
  ...
  RouterRegister: app => {
    ...
    for (let obj of funcBundler) {
      ...
      for (let req of obj.routers) {
        ...
        //     app.logger.info(`[egg-swagger-docs] validate ${router.ruleName}`);
        ...
        // }
        app.logger.info(`[egg-swagger-docs] register router: ${req.method} ${req.route} for ${direct.replace(path.sep, '-')}-${req.func} `);
      }
