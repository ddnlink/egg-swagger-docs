    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        if (routers) {
          ...
          if (routers[0][1].toLowerCase() == 'post' || routers[0][1].toLowerCase() == 'put') {
            const body = generateRequest(blocks[i], routers, components, config);
            path_method.requestBody = body[0];
          } else {
            path_method.parameters = generateRequest(blocks[i], routers, components, config);
          }
